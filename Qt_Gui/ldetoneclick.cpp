...
LDETOneClick::LDETOneClick(QWidget *parent)
    : QWidget(parent)
{
    initUI();
    initWorkerThread();
...
LDETOneClick::~LDETOneClick()
{
    cleanupWorkerThread();
    closeProgressDialog();
}

bool LDETOneClick::isRunning() const
{
...
void LDETOneClick::initTitleArea()
{
...
void LDETOneClick::initFormArea()
{
    ...
    m_roomIdEdit = new LDETLineEdit(m_formWidget);
    ...
    m_hostIpEdit = new LDETLineEdit(m_formWidget);
    ...
    // 一键联机按钮
    m_oneClickBtn = new LDETPushBtn(this);
...
void LDETOneClick::initServerArea()
{
    ...
    // 服务器地址输入框
    m_serverEdit = new LDETLineEdit(m_serverWidget);

    // 添加按钮
    m_addServerBtn = new LDETPushBtn(m_serverWidget);
    ...
    // 删除按钮
    m_removeServerBtn = new LDETPushBtn(m_serverWidget);
    ...
    // 公共服务器列表按钮
    m_publicServerBtn = new LDETPushBtn(m_serverWidget);
    ...
    // 从公共服务器配置文件加载默认服务器
    QString publicServersPath = LDETSettings::getPublicServersPath();
...
void LDETOneClick::initButtonArea()
{
    m_bottomWidget = new QWidget(this);
    QGridLayout *bottomLayout = new QGridLayout(m_bottomWidget);

    // 我做房主（默认不勾选，即房客模式）
    m_hostModeCheckBox = new LDETCheckBtn(m_bottomWidget);
    ...
    // 低延迟优先
    m_latencyFirstCheckBox = new LDETCheckBtn(m_bottomWidget);
...
void LDETOneClick::setupConnections()
{
    // 添加服务器按钮
    connect(m_addServerBtn, &QPushButton::clicked, this, &LDETOneClick::onAddServer);
    
    // 输入框回车添加
    connect(m_serverEdit, &QLineEdit::returnPressed, this, &LDETOneClick::onServerEditReturnPressed);
    
    // 删除服务器按钮
    connect(m_removeServerBtn, &QPushButton::clicked, this, &LDETOneClick::onRemoveServer);
    
    // 服务器列表选中变化
    connect(m_serverListWidget, &QListWidget::itemSelectionChanged, 
            this, &LDETOneClick::onServerSelectionChanged);
    
    // 一键联机按钮
    connect(m_oneClickBtn, &QPushButton::clicked, this, &LDETOneClick::onOneClickBtnClicked);
    
    // 房主模式开关
    connect(m_hostModeCheckBox, &LDETCheckBtn::toggled, this, &LDETOneClick::onHostModeChanged);
...
void LDETOneClick::initWorkerThread()
{
    ...
    connect(m_worker, &ETRunWorker::etRunStarted, 
            this, &LDETOneClick::onNetworkStarted, Qt::QueuedConnection);
    connect(m_worker, &ETRunWorker::etRunStopped, 
            this, &LDETOneClick::onNetworkStopped, Qt::QueuedConnection);
    connect(m_worker, &ETRunWorker::infosCollected, 
            this, &LDETOneClick::onInfosCollected, Qt::QueuedConnection);
    ...
    connect(m_monitorTimer, &QTimer::timeout, this, &LDETOneClick::onMonitorTimerTimeout);
    
    // 初始化连接超时定时器
    m_connectionTimer = new QTimer(this);
    m_connectionTimer->setSingleShot(true);
    m_connectionTimer->setInterval(CONNECTION_TIMEOUT_MS);
    connect(m_connectionTimer, &QTimer::timeout, this, &LDETOneClick::onConnectionTimeout);
}

void LDETOneClick::cleanupWorkerThread()
{
    // 停止监测定时器
    if (m_monitorTimer) {
        m_monitorTimer->stop();
    ...
    if (m_workerThread) {
        ...
        if (isRunning() && m_worker) {
            QMetaObject::invokeMethod(m_worker, "stopNetwork", 
                                      ...
                                      Q_ARG(std::string, "LDET-OneClick"));
        }
...
QString LDETOneClick::base32Encode(const QByteArray& data)
{
    QString result;
    int buffer = 0;
    int bitsLeft = 0;
...
QByteArray LDETOneClick::base32Decode(const QString& encoded)
{
    QByteArray result;
    int buffer = 0;
    int bitsLeft = 0;
...
QPair<QString, QString> LDETOneClick::generateRoomCredentials()
{
    ...
    QString networkId = QStringLiteral("LDET-OneClick-") + QString::fromLatin1(networkIdRaw);
    QString password = QString::fromLatin1(passwordRaw);

    return qMakePair(networkId, password);
}

QString LDETOneClick::encodeConnectionCode(const QString& networkId, const QString& password)
{
    QString networkPureId = networkId;
    networkPureId.remove(QStringLiteral("LDET-OneClick-"));
...
QPair<QString, QString> LDETOneClick::decodeConnectionCode(const QString& code)
{
    // 移除所有非Base32字符并转为大写
    ...
    return qMakePair(QStringLiteral("LDET-OneClick-") + networkPureId, password);
}

// ==================== TOML 配置生成 ====================

std::string LDETOneClick::generateHostTomlConfig()
{
    // 生成房间凭证
    ...
    toml << "instance_name = \"LDET-OneClick\"\n";
    toml << "hostname = \"host\"\n";
    toml << "dhcp = false\n";
...
std::string LDETOneClick::generateGuestTomlConfig()
{
...
#ifdef QT_DEBUG
    std::clog << "[LDETOneClick] 房客解码信息: 网络号="
...
void LDETOneClick::updateInterfaceState()
{
...
void LDETOneClick::showProgressDialog(const QString& title)
{
    if (m_progressDialog) {
...
void LDETOneClick::closeProgressDialog()
{
    m_connectionTimer->stop();
...
void LDETOneClick::onServerEditReturnPressed()
{
    onAddServer();
}

void LDETOneClick::onRemoveServer()
{
...
void LDETOneClick::onServerSelectionChanged()
{
    bool hasSelection = m_serverListWidget->currentRow() >= 0;
    m_removeServerBtn->setEnabled(hasSelection);
}

void LDETOneClick::onHostModeChanged(bool checked)
{
    if (checked) {
        // 房主模式：显示联机码（只读）
        m_roomIdLabel->setText(QStringLiteral("联机码："));
...
void LDETOneClick::onOneClickBtnClicked()
{
    // 如果正在停止中，不处理
    ...
    try {
...
#ifdef QT_DEBUG
            std::clog << "[LDETOneClick] 房主模式 联机码: "
        ...
        // 输出 TOML 配置用于调试
#ifdef QT_DEBUG
        std::clog << "[LDETOneClick] 运行网络前 TOML 配置:" << std::endl
                  << tomlConfig << std::endl;
#endif
...
void LDETOneClick::stopCurrentNetwork()
{
...
void LDETOneClick::onNetworkStarted(const std::string &instName, bool success, const std::string &errorMsg)
{
    if (instName != "LDET-OneClick") {
        return;
    }
...
void LDETOneClick::onNetworkStopped(const std::string &instName, bool success, const std::string &errorMsg)
{
    if (instName != "LDET-OneClick") {
        return;
    }
...
void LDETOneClick::onInfosCollected(const std::vector<EasyTierFFI::KVPair> &infos)
{
    for (const auto &info : infos) {
        if (info.key == "LDET-OneClick") {
            // 解析 JSON
            QJsonDocument doc = QJsonDocument::fromJson(QByteArray::fromStdString(info.value));
            // 打印 JSON 内容用于调试
#ifdef QT_DEBUG
            std::clog << "[LDETOneClick] 收到 JSON 内容: "
            ...
            // 输出调试信息
#ifdef QT_DEBUG
            std::clog << "[LDETOneClick] 收到节点信息, 直连节点数=" << peers.size()
...
void LDETOneClick::parsePeerInfo(const QJsonArray& peers, const QJsonArray& routes, const QJsonObject& root)
{
    ...
    if (m_currentRole == UserRole::Host) {
...
#ifdef QT_DEBUG
            std::clog << "[LDETOneClick] 房主本机IP: " << localIp.toStdString() << std::endl;
#endif
        }
...
#ifdef QT_DEBUG
        std::clog << "[LDETOneClick] 当前联机人数: " << count << std::endl;
#endif
    }
        ...
        if (hostIp != m_lastHostIp) {
            ...
            m_lastHostIp = hostIp;
#ifdef QT_DEBUG
            std::clog << "[LDETOneClick] 房主IP: " << hostIp.toStdString() << std::endl;
#endif
        }
    }
}

void LDETOneClick::onConnectionTimeout()
{
...
void LDETOneClick::onMonitorTimerTimeout()
{
    if (m_worker) {
