...
LDETServers::LDETServers(QWidget *parent)
    : QWidget(parent)
{
    initUI();
    setupConnections();
...
LDETServers::~LDETServers()
{
    saveServers();
}

void LDETServers::initUI()
{
...
void LDETServers::initTitleArea()
{
...
void LDETServers::initListArea()
{
    // 服务器列表
    m_serverListWidget = new LDETLabelList(this);
...
void LDETServers::initButtonArea()
{
    ...
    // 添加服务器按钮
    m_addBtn = new LDETPushBtn(m_bottomWidget);
...
void LDETServers::setupConnections()
{
    // 添加按钮
    connect(m_addBtn, &QPushButton::clicked, this, &LDETServers::onAddServer);

    // 列表双击
    connect(m_serverListWidget, &LDETLabelList::itemDoubleClicked,
            this, [this](LDETLabelListItem *item) {
                Q_UNUSED(item)
                onEditServer();
            });
    ...
    connect(m_serverListWidget, &QWidget::customContextMenuRequested,
            this, &LDETServers::onShowContextMenu);

    // 右键菜单动作
    connect(m_editAction, &QAction::triggered, this, &LDETServers::onEditServer);
    connect(m_deleteAction, &QAction::triggered, this, &LDETServers::onDeleteServer);
}

void LDETServers::loadServers()
{
...
void LDETServers::updateList()
{
    m_serverListWidget->clear();

    for (const auto &server : m_servers) {
        auto *item = new LDETLabelListItem();
        // 显示格式：名称 (地址)
...
QString LDETServers::configFilePath() const
{
    return LDETSettings::getConfigPath() + "/servers.json";
}

void LDETServers::onAddServer()
{
...
void LDETServers::onDeleteServer()
{
    int currentRow = m_serverListWidget->currentRow();
    if (currentRow < 0 || currentRow >= static_cast<int>(m_servers.size())) {
        return;
...
void LDETServers::onShowContextMenu(const QPoint &pos)
{
    // 检查是否有选中项
    LDETLabelListItem *item = m_serverListWidget->itemAt(pos);
    if (item) {
