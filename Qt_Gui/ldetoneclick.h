...
class LDETOneClick : public QWidget
{
    Q_OBJECT

public:
    explicit LDETOneClick(QWidget *parent = nullptr);
    ~LDETOneClick() override;
...
private:
    ...
    LDETLineEdit *m_roomIdEdit = nullptr;      ///< 联机码输入/显示框
    LDETLineEdit *m_hostIpEdit = nullptr;      ///< 房主虚拟 IP 显示框
    QLabel *m_roomIdLabel = nullptr;        ///< 联机码标签
    QLabel *m_hostIpLabel = nullptr;        ///< 房主虚拟 IP 标签

    // 一键联机按钮
    LDETPushBtn *m_oneClickBtn = nullptr;

    // 服务器地址区域
    QWidget *m_serverWidget = nullptr;
    LDETLineEdit *m_serverEdit = nullptr;      ///< 服务器地址输入框
    LDETPushBtn *m_addServerBtn = nullptr;  ///< 添加服务器按钮
    QListWidget *m_serverListWidget = nullptr; ///< 服务器列表
    LDETPushBtn *m_removeServerBtn = nullptr;  ///< 删除服务器按钮
    LDETPushBtn *m_publicServerBtn = nullptr;  ///< 公共服务器列表按钮

    // 底部按钮区域
    QWidget *m_bottomWidget = nullptr;
    LDETCheckBtn *m_hostModeCheckBox = nullptr;  ///< 我做房主开关
    LDETCheckBtn *m_latencyFirstCheckBox = nullptr; ///< 低延迟优先开关
