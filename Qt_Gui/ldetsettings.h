 ...
 * - 手动检查更新
 */
class LDETSettings : public QWidget
{
    Q_OBJECT

public:
    explicit LDETSettings(QWidget *parent = nullptr);
    ~LDETSettings() override = default;
...
private:
    ...
    // ========== 设置项控件 ==========
    LDETCheckBtn *m_hideOnTrayCheckBox = nullptr;      ///< 关闭时隐藏到托盘
    LDETCheckBtn *m_autoReconnectCheckBox = nullptr;   ///< 自动回连
    LDETCheckBtn *m_autoCheckUpdateCheckBox = nullptr; ///< 自动检查更新
    LDETCheckBtn *m_autoStartCheckBox = nullptr;       ///< 开机自启

    // ========== 版本显示控件 ==========
    QLabel *m_versionLabel = nullptr;               ///< LDEasyTier 版本标签
    LDETPushBtn *m_checkUpdateBtn = nullptr;        ///< 检查更新按钮

    // ========== 操作按钮 ==========
    LDETPushBtn *m_discardBtn = nullptr;            ///< 丢弃按钮
    LDETPushBtn *m_saveBtn = nullptr;               ///< 保存按钮
