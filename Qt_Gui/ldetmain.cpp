...
LDETMain::LDETMain(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::LDETMain)
{
    ui->setupUi(this);
...
// =============== 初始化调色板 ===============
    ...
    connect(hints, &QStyleHints::colorSchemeChanged, this, &LDETMain::onSchemeChanged);
...
// =============== 连接信号槽 ===============
    ...
    {
        QDesktopServices::openUrl(QUrl("https://ldet.cn/docs-home"));
    });
    connect(m_aboutUsBtn, &QPushButton::clicked, this, [=, this]()
    {
        QDesktopServices::openUrl(QUrl("https://ldet.cn"));
    });
    ...
    {
        QDesktopServices::openUrl(QUrl("https://ldet.cn/other/donate/"));
    });
...
void LDETMain::closeEvent(QCloseEvent *event)
{
    // 根据缓存设置决定是否隐藏到托盘
    if (m_hideOnTray) {
...
/// @brief 初始化系统托盘
void LDETMain::initTrayIcon()
{
    // 创建托盘图标
    m_trayIcon = new QSystemTrayIcon(this);
    m_trayIcon->setIcon(QIcon(":/icons/icon.ico"));
    m_trayIcon->setToolTip(tr("LDEasyTier"));
    ...
    connect(m_showAction, &QAction::triggered, this, &LDETMain::onShowWindow);
    m_trayMenu->addAction(m_showAction);
    
    // 隐藏窗口动作
    m_hideAction = new QAction(tr("隐藏窗口"), this);
    m_hideAction->setIcon(QIcon(":/icons/eye-slash.svg"));
    connect(m_hideAction, &QAction::triggered, this, &LDETMain::onHideWindow);
    ...
    connect(m_quitAction, &QAction::triggered, this, &LDETMain::onQuitApp);
    ...
    // 连接托盘图标激活信号
    connect(m_trayIcon, &QSystemTrayIcon::activated, this, &LDETMain::onTrayIconActivated);
...
/// @brief 托盘图标激活处理
void LDETMain::onTrayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
    switch (reason) {
...
/// @brief 处理系统调色板变化
void LDETMain::onSchemeChanged(const Qt::ColorScheme &scheme)
{
...
void LDETMain::initHelloPage()
{
    ...
    QLabel *titleLabel = new QLabel("LDEasyTier", this);
    ...
    m_aboutUsBtn = new LDETPushBtn("关于项目", this);
    m_aboutUsBtn->setFixedWidth(125);
    m_aboutETBtn = new LDETPushBtn("关于 EasyTier", this);
    m_aboutETBtn->setFixedWidth(125);
    m_donateBtn = new LDETPushBtn("捐赠", this);
    m_donateBtn->setFixedWidth(125);
    m_notClickBtn = new LDETPushBtn("千万别点", this);
...
void LDETMain::initNetworkPage()
{
    // 初始化网络界面
    m_networkPage = new LDETNetwork(this);
    ...
    if (LDETSettings::isAutoReconnect() && !wasRunningIndexes.isEmpty()) {
        // 延迟启动网络，确保界面已完全初始化
...
void LDETMain::initOneClickPage()
{
    // 初始化一键组网界面
    m_oneClickPage = new LDETOneClick(this);
...
void LDETMain::initServersPage()
{
    // 初始化服务器收藏界面
    m_serversPage = new LDETServers(this);
...
void LDETMain::initSettingsPage()
{
    // 初始化设置界面
    m_settingsPage = new LDETSettings(this);
    ...
    // 连接隐藏到托盘设置变更信号
    connect(m_settingsPage, &LDETSettings::hideOnTrayChanged, this, &LDETMain::onHideOnTrayChanged);

    // 初始化缓存的设置值
    m_hideOnTray = LDETSettings::isHideOnTray();
}

void LDETMain::onHideOnTrayChanged(bool hideOnTray)
{
    // 更新缓存的设置值
...
void LDETMain::saveCurrentPage()
{
    LDETSettings::saveLastPage(getCurrentPageName());
}

void LDETMain::restoreLastPage()
{
    const QString pageName = LDETSettings::loadLastPage();
...
QString LDETMain::getCurrentPageName() const
{
