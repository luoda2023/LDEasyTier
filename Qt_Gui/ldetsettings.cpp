...
LDETSettings::LDETSettings(QWidget *parent)
    : QWidget(parent)
{
    // 加载设置
    loadSettings();
...
void LDETSettings::initUI()
{
    ...
    // 创建开关按钮
    m_hideOnTrayCheckBox = new LDETCheckBtn(tr("关闭时隐藏到托盘"), this);
    m_autoReconnectCheckBox = new LDETCheckBtn(tr("自动回连"), this);
    m_autoCheckUpdateCheckBox = new LDETCheckBtn(tr("自动检查更新"), this);
    m_autoStartCheckBox = new LDETCheckBtn(tr("开机自启"), this);
    ...
    // 构建版本字符串
    QString versionStr = tr("LDEasyTier 版本：") + QString::fromUtf8(PROJECT_VERSION);
#if IS_BETA_VERSION
    versionStr += " beta";
#endif
    ...
    m_checkUpdateBtn = new LDETPushBtn(tr("检查更新"), this);
    ...
    m_discardBtn = new LDETPushBtn(tr("丢弃"), this);
    m_discardBtn->setFixedWidth(80);
    m_discardBtn->setEnabled(false);

    m_saveBtn = new LDETPushBtn(tr("保存"), this);
...
void LDETSettings::setupConnections()
{
    // 复选框状态变化
    connect(m_hideOnTrayCheckBox, &QCheckBox::checkStateChanged, this, &LDETSettings::onCheckBoxStateChanged);
    connect(m_autoReconnectCheckBox, &QCheckBox::checkStateChanged, this, &LDETSettings::onCheckBoxStateChanged);
    connect(m_autoCheckUpdateCheckBox, &QCheckBox::checkStateChanged, this, &LDETSettings::onCheckBoxStateChanged);
    connect(m_autoStartCheckBox, &QCheckBox::checkStateChanged, this, &LDETSettings::onCheckBoxStateChanged);

    // 按钮点击
    connect(m_saveBtn, &QPushButton::clicked, this, &LDETSettings::onSaveButtonClicked);
    connect(m_discardBtn, &QPushButton::clicked, this, &LDETSettings::onDiscardButtonClicked);
    connect(m_checkUpdateBtn, &QPushButton::clicked, this, &LDETSettings::onCheckUpdateButtonClicked);
}

// ==================== UI 事件处理 ====================

void LDETSettings::onCheckBoxStateChanged()
{
    updateButtonState();
}

void LDETSettings::onSaveButtonClicked()
{
    // 处理开机自启设置变更
...
void LDETSettings::onDiscardButtonClicked()
{
    // 恢复到上次保存的设置
    discardChanges();
}

void LDETSettings::onCheckUpdateButtonClicked()
{
    checkForUpdate(this, false);
}

void LDETSettings::updateButtonState()
{
...
QString LDETSettings::getConfigPath()
{
...
QString LDETSettings::getPublicServersPath()
{
    return QCoreApplication::applicationDirPath() + "/publicservers.json";
}

QJsonObject LDETSettings::loadSettingsFromFile()
{
...
bool LDETSettings::saveSettingsToFile(const QJsonObject &settings)
{
    QString configPath = getConfigPath();
    QDir dir(configPath);
...
void LDETSettings::loadSettings()
{
...
void LDETSettings::saveSettings()
{
    // 从 UI 读取当前值
...
void LDETSettings::discardChanges()
{
    // 恢复复选框状态
...
bool LDETSettings::hasUnsavedChanges() const
{
...
bool LDETSettings::setAutoStart(bool enable)
{
    ...
    if (enable) {
        ...
        QString desktopContent = QString(
            ...
            "Exec=%2 --auto-start\n"
            "Icon=%1\n"
            "Comment=LDEasyTier - EasyTier GUI Client\n"
            "Terminal=false\n"
            "Categories=Network;\n"
        ).arg(appName, appPath);
...
void LDETSettings::saveLastPage(const QString &pageName)
{
    QJsonObject settings = loadSettingsFromFile();
...
QString LDETSettings::loadLastPage()
{
    const QJsonObject settings = loadSettingsFromFile();
