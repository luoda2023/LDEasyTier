...
private:
    LDETLineEdit *m_nameEdit = nullptr;        ///< 服务器名称输入框
    LDETLineEdit *m_addressEdit = nullptr;     ///< 服务器地址输入框
};

/// @brief 服务器收藏页面
class LDETServers : public QWidget
{
    Q_OBJECT

public:
    explicit LDETServers(QWidget *parent = nullptr);
    ~LDETServers() override;
...
private:
    ...
    // 列表区域
    LDETLabelList *m_serverListWidget = nullptr;

    // 底部按钮区域
    QWidget *m_bottomWidget = nullptr;
    LDETPushBtn *m_addBtn = nullptr;        ///< 添加服务器按钮
