...
LDETServersDialog::LDETServersDialog(QWidget *parent)
    : QDialog(parent)
{
    initUI();
    loadServers();
...
void LDETServersDialog::initUI()
{
    ...
    LDETSmoothScroll::install(m_scrollArea);
    ...
    m_okBtn = new LDETPushBtn(tr("确定"), this);
    m_okBtn->setMinimumWidth(80);
    m_cancelBtn = new LDETPushBtn(tr("取消"), this);
    m_cancelBtn->setMinimumWidth(80);

    m_selectAllBtn = new LDETPushBtn(tr("全选"), this);
    m_selectAllBtn->setMinimumWidth(80);
    m_deselectAllBtn = new LDETPushBtn(tr("全不选"), this);
    ...
    connect(m_selectAllBtn, &QPushButton::clicked, this, &LDETServersDialog::onSelectAll);
    connect(m_deselectAllBtn, &QPushButton::clicked, this, &LDETServersDialog::onDeselectAll);
...
void LDETServersDialog::updateColorScheme()
{
...
void LDETServersDialog::loadServers()
{
...
void LDETServersDialog::loadPublicServers()
{
...
void LDETServersDialog::clearServerCheckboxes()
{
...
void LDETServersDialog::updateServerList()
{
    clearServerCheckboxes();

    for (const auto &server : m_servers) {
        auto *checkBox = new LDETCheckBtn(m_scrollContent);
        if (server.isPublic) {
...
QString LDETServersDialog::serverFilePath() const
{
    return LDETSettings::getConfigPath() + "/servers.json";
}

QString LDETServersDialog::publicServerFilePath() const
{
    return LDETSettings::getPublicServersPath();
}

void LDETServersDialog::setSelectedServers(const QStringList &selectedAddresses)
{
...
QStringList LDETServersDialog::selectedServers() const
{
    QStringList result;
...
void LDETServersDialog::onSelectAll()
{
    for (auto *checkBox : m_serverCheckBoxes) {
        checkBox->setChecked(true);
    }
...
void LDETServersDialog::onDeselectAll()
{
    for (auto *checkBox : m_serverCheckBoxes) {
...
void LDETServersDialog::paintEvent(QPaintEvent *event)
{
