...
LDETResponsiveGrid::LDETResponsiveGrid(QWidget *parent)
    : QWidget(parent)
...
LDETResponsiveGrid::~LDETResponsiveGrid() = default;

void LDETResponsiveGrid::addItem(QWidget *widget)
{
    if (!widget) {
        return;
    }
...
void LDETResponsiveGrid::removeItem(QWidget *widget)
{
    if (!widget) {
        return;
    }
...
void LDETResponsiveGrid::clearItems()
{
...
void LDETResponsiveGrid::setMinColumns(int columns)
{
...
int LDETResponsiveGrid::minColumns() const
{
    return m_minColumns;
}

void LDETResponsiveGrid::setMaxColumns(int columns)
{
    m_maxColumns = qMax(m_minColumns, columns);
    recalculateLayout();
}

int LDETResponsiveGrid::maxColumns() const
{
    return m_maxColumns;
}

int LDETResponsiveGrid::currentColumns() const
{
    return m_currentColumns;
}

int LDETResponsiveGrid::itemCount() const
{
    return static_cast<int>(m_items.size());
}

void LDETResponsiveGrid::setGridSpacing(int horizontal, int vertical)
{
    m_gridLayout->setHorizontalSpacing(horizontal);
    m_gridLayout->setVerticalSpacing(vertical);
}

void LDETResponsiveGrid::setGridMargins(int left, int top, int right, int bottom)
{
    m_gridLayout->setContentsMargins(left, top, right, bottom);
}

void LDETResponsiveGrid::setSafetyRatio(qreal ratio)
{
    m_safetyRatio = qMax(0.8, ratio);
    recalculateLayout();
}

QSize LDETResponsiveGrid::sizeHint() const
{
    return m_gridLayout->sizeHint();
}

QSize LDETResponsiveGrid::minimumSizeHint() const
{
    return QSize(requiredWidthForColumns(m_minColumns), m_gridLayout->minimumSize().height());
}

void LDETResponsiveGrid::resizeEvent(QResizeEvent *event)
{
    recalculateLayout();
    QWidget::resizeEvent(event);
}

void LDETResponsiveGrid::recalculateLayout()
{
    if (m_items.isEmpty()) {
        return;
    }
...
int LDETResponsiveGrid::requiredWidthForColumns(int columns) const
{
