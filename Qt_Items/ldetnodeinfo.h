...
class LDETNodeInfo : public QWidget
{
    Q_OBJECT
    Q_PROPERTY(qreal borderOpacity READ borderOpacity WRITE setBorderOpacity)

public:
    explicit LDETNodeInfo(QWidget *parent = nullptr);
    explicit LDETNodeInfo(const NodeInfo &info, QWidget *parent = nullptr);
    ~LDETNodeInfo() override = default;
