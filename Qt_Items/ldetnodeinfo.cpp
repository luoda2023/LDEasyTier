...
LDETNodeInfo::LDETNodeInfo(QWidget *parent)
    : QWidget(parent)
...
LDETNodeInfo::LDETNodeInfo(const NodeInfo &info, QWidget *parent)
    : QWidget(parent)
    , m_nodeInfo(info)
    , m_borderAnimation(nullptr)
{
...
void LDETNodeInfo::updateColorScheme()
{
...
QColor LDETNodeInfo::getConnTypeColor(NodeConnType type)
{
    switch (type) {
    case NodeConnType::Direct: return LDETTheme::ConnDirect;
    case NodeConnType::Relay: return LDETTheme::ConnRelay;
    case NodeConnType::Server: return LDETTheme::ConnServer;
    }
    return LDETTheme::ConnDirect;
}

QString LDETNodeInfo::getConnTypeText(NodeConnType type)
{
    switch (type) {
...
NodeInfo LDETNodeInfo::nodeInfo() const
{
    return m_nodeInfo;
}

void LDETNodeInfo::setNodeInfo(const NodeInfo &info)
{
    m_nodeInfo = info;
    update();
}

qreal LDETNodeInfo::borderOpacity() const { return m_borderOpacity; }
void LDETNodeInfo::setBorderOpacity(qreal opacity)
{
...
QSize LDETNodeInfo::sizeHint() const
{
    return QSize(300, WIDGET_HEIGHT);
}

QSize LDETNodeInfo::minimumSizeHint() const
{
    return QSize(250, WIDGET_HEIGHT);
}

int LDETNodeInfo::calculateLabelsWidth() const
{
...
void LDETNodeInfo::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event)
    ...
    QColor finalBorderColor = LDETDrawUtils::blendColors(m_borderColor, m_highlightBorderColor, m_borderOpacity);

    LDETDrawUtils::drawRoundedRect(&painter, borderRect, BORDER_RADIUS, m_bgColor, finalBorderColor, 1);
    ...
    if (m_nodeInfo.isLocalNode) {
        ...
        painter.fillPath(localPath, LDETTheme::ConnLocal);
...
void LDETNodeInfo::enterEvent(QEnterEvent *event)
{
...
void LDETNodeInfo::leaveEvent(QEvent *event)
{
...
void LDETNodeInfo::mousePressEvent(QMouseEvent *event)
{
...
void LDETNodeInfo::contextMenuEvent(QContextMenuEvent *event)
{
    bool showIp = !m_nodeInfo.virtualIp.isEmpty() && m_nodeInfo.virtualIp != "0.0.0.0";
