...
class LDETComboPopup;
class QListView;

/// @brief 自定义下拉选择框控件
/// 基于 QComboBox 派生，完全自绘（圆角边框、悬停动画、按下态、禁用态）
class LDETComboBox : public QComboBox
{
    Q_OBJECT
    Q_PROPERTY(qreal borderOpacity READ borderOpacity WRITE setBorderOpacity)

public:
    explicit LDETComboBox(QWidget *parent = nullptr);
    ~LDETComboBox() override;
...
private:
    ...
    LDETComboPopup *m_popup = nullptr;
