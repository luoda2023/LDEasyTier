...
class LDETComboDelegate : public QStyledItemDelegate
{
public:
    explicit LDETComboDelegate(QObject *parent = nullptr)
...
class LDETComboPopup : public QFrame
{
    Q_OBJECT

public:
    explicit LDETComboPopup(QWidget *parent = nullptr)
        : QFrame(parent, Qt::Popup | Qt::FramelessWindowHint)
    {
        ...
        m_view->setItemDelegate(new LDETComboDelegate(m_view));
        layout->addWidget(m_view);

        connect(m_view, &QAbstractItemView::clicked, this, &LDETComboPopup::itemClicked);
        connect(m_view, &QAbstractItemView::activated, this, &LDETComboPopup::itemClicked);
    }
...
protected:
    void paintEvent(QPaintEvent *event) override
    {
        ...
        LDETDrawUtils::drawRoundedRect(&painter, drawRect, POPUP_RADIUS, m_bgColor, m_borderColor, 1);
    }
...
LDETComboBox::LDETComboBox(QWidget *parent)
    : QComboBox(parent)
    , m_isPressed(false)
{
    init();
...
void LDETComboBox::updateColorScheme()
{
    auto *theme = LDETTheme::instance();
...
qreal LDETComboBox::borderOpacity() const
{
    return m_borderOpacity;
}

void LDETComboBox::setBorderOpacity(qreal opacity)
{
...
void LDETComboBox::startBorderAnimation(qreal targetOpacity)
{
...
QSize LDETComboBox::sizeHint() const
{
    QFontMetrics fm(font());
...
QSize LDETComboBox::minimumSizeHint() const
{
    return QSize(80, sizeHint().height());
}

void LDETComboBox::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event)
...
void LDETComboBox::enterEvent(QEnterEvent *event)
{
...
void LDETComboBox::leaveEvent(QEvent *event)
{
...
void LDETComboBox::mousePressEvent(QMouseEvent *event)
{
...
void LDETComboBox::mouseReleaseEvent(QMouseEvent *event)
{
...
void LDETComboBox::changeEvent(QEvent *event)
{
...
void LDETComboBox::showPopup()
{
    ...
    // 创建或复用弹出窗口
    if (!m_popup) {
        m_popup = new LDETComboPopup(nullptr);
        m_popup->updateTheme();

        connect(m_popup, &LDETComboPopup::itemClicked, this, &LDETComboBox::onPopupItemClicked);
        connect(m_popup, &LDETComboPopup::popupHidden, this, [this]() {
            m_isPressed = false;
            update();
        });
    }
...
void LDETComboBox::keyPressEvent(QKeyEvent *event)
{
...
void LDETComboBox::onPopupItemClicked(const QModelIndex &index)
{
    if (index.isValid()) {
