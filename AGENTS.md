...
LDEasyTier (LUODA异地组网) 是一个基于 Qt C++ 框架开发的异地组网工具，后端核心为 EasyTier（去中心化组网方案）。
...
- **主窗口**: `Qt_Gui/ldetmain.h/cpp` 管理 `QStackedWidget`，包含欢迎页、网络配置页、一键联机页、服务器收藏页、设置页。
...
- **必须在链接前终止运行中的进程**，否则 `Permission denied`：
  ```
  taskkill /F /IM LDEasyTier.exe; cmake --build cmake-build-debug
  ```
...
| `Qt_Gui/ldetnetwork.h` | 添加 UI 控件成员（`LDETCheckBtn*` 或 `LDETLineEdit*` 等） |
| `Qt_Gui/ldetnetwork.cpp` | 构造函数初始化 / `initAdvancedSettingsPage()` / `setupUIConnections()` / `loadConfToUI()`（blockSignals → setValue → unblock） |

**控件布局规则**：
- `LDETCheckBtn` 放入 `FunctionSection.checkBoxes` 列表，由 `updateFunctionGridLayout()` 统一动态排版（2-5 列自适应)。
- 非 CheckBtn 控件（`LDETLineEdit`、`LDETComboBox`、列表区）使用独立的 `QHBoxLayout` 或 `QVBoxLayout` 行，**不与动态网格混合**。
